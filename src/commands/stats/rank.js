...
module.exports = {
  name: "rank",
  description: "Affiche le classement des membres dans ce serveur",
  cooldown: 5,
  category: "STATS",
...
async function getRank({ guild }, member, settings) {
  ...
  if (!memberStats.xp) return `${user.username} n'est pas encore classé !`;
  ...
  url.searchParams.append("nom", user.username);
  ...
  if (!response.success) return "Échec de la carte de rang";
