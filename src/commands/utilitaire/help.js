...
module.exports = {
  name: "help",
  description: "menu d'aide de commande",
  category: "UTILITAIRE",
  ...
  async messageRun(message, args, data) {
    ...
    await message.safeReply("Aucune commande correspondante trouvée");
  },
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Afficher les commandes de la catégorie ${v.name}`,
      emoji: v.emoji,
    });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Information sur moi : **\n" +
        `Bonjour je me présente je suis ${guild.members.me.displayName} !\n` +
        "Un BOT discord polyvalent cool qui peut répondre à tous vos besoins\n\n" +
        `**Personne d'assistance :**Zelloxx`
    );
...
function getSlashCategoryEmbeds(client, category) {
  ...
  while (commands.length) {
    ...
    toAdd = toAdd.map((cmd) => {
      ...
      return `\`/${cmd.name}\`\n ❯ **Description** : ${cmd.description}\n ${
        !subCmds?.length ? "" : `❯ **Sous-commande [${subCmds?.length}]** : ${subCmdsString}\n`
      } `;
    });
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `Page ${index + 1} à ${arrSplitted.length}` });
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  if (category === "IMAGE") {
    ...
    collector +=
      ...
      `**${prefix}cmd:** Choisit les auteurs de messages Avatar comme image\n` +
      `**${prefix}cmd <@member> :** Picks mentionné les membres Avatar comme image\n` +
      `**${prefix}cmd <url> :** Choisit l'image de l'URL fournie\n` +
      `**${prefix}cmd [attachment] :** Picks Image de l'attachement`;
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `Page ${index + 1} à ${arrSplitted.length} | Taper ${prefix}help <command> Pour plus d'informations sur la command`,
      });
    arrEmbeds.push(embed);
  });
