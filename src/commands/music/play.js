...
async function play({ member, guild, channel }, query) {
  if (!member.voice.channel) return "🚫 Vous devez d'abord rejoindre un salon vocale";
  ...
  try {
    if (guild.client.musicManager.spotify.isSpotifyUrl(query)) {
      if (!process.env.SPOTIFY_CLIENT_ID || !process.env.SPOTIFY_CLIENT_SECRET) {
        return "🚫 Les chansons de Spotify ne peuvent pas être jouées. Veuillez contacter le propriétaire du bot";
      }
      ...
      switch (item?.type) {
        ...
        default:
          return "🚫 Une erreur s'est produite lors de la recherche de la chanson";
      }
  ...
  if (!tracks) return "🚫 Une erreur s'est produite lors de la recherche de la chanson";

  if (tracks.length === 1) {
    const track = tracks[0];
    if (!player?.playing && !player?.paused && !player?.queue.tracks.length) {
      embed.setAuthor({ name: "Ajout de piste à la file d'attente" });
    } else {
      const fields = [];
      embed
        .setAuthor({ name: "Ajout de piste à la file d'attente" })
